...
class BasicClassifier(object):
    ...
    def train(self, X, y, learning_rate=1e-3, reg=1e-5, num_iters=100,
        ...
        for it in range(num_iters):
            ...
            batch_index = np.random.choice(range(X.shape[0]), batch_size, replace=False) 
            ...
            #                     START OF YOUR CODE               #
            #########################################################################
            loss, dW = self.loss(X, y, reg) #get loss and gradient
            loss_history.append(loss) #add loss to loss history
            self.W -= learning_rate * dW # [K x D] #adjust weights
    ...
    def predict(self, X):
        ...
        probability_scores = X.dot(self.W)
        y_pred = np.argmax(probability_scores, axis=1)
