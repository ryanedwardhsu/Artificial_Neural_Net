...
def affine_forward(x, w, b):
    ...
    N = x.shape[0]
    dim = x.shape[1]
    out = x.dot(w) + b.reshape(w.shape[1])
...
def affine_backward(dout, x, w, b):
    ...
    # note that out = w*x = ix * b where ix is [N, 1] of all 1's
    db = np.sum(dout, axis=0) # db = ix.T * dout which sums dout across axis 0
...
def softmax_loss(x, y):
    ...
    dim = x.shape[1]
    scores = x
    scores = (scores.T - np.max(scores, axis=1).T).T #subtract max score of observation
